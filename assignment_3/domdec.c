...
printf("My rank %d of %d\n", rank, size );
        ...
        for (i=0; i<nxn_loc; i++){
	  if (i == 0){
	    printf("The first ghost cells for rank = %d are for i = %d, f[0] = %f \n ",rank, i, f[0]);
	  }
	  else if (i == nxn_loc -1){
	    printf("The last ghost cells for rank = %d  are for i = %d f[nxn_loc-1] = %f \n ",rank, i,  f[nxn_loc-1]);
	    }
    else if (i == nxn_loc -1){
	    ...
	    printf("For f[i] = %f, dfdx = %f and cos = %f  \n", f[i], dfdx[i], cos(L_loc*rank + (i-1) * dx));
	}	  
	}
    MPI_Finalize();
}
